...
public class PlayerMeleeAttacks : MonoBehaviour
{
    ...
    public List<Enemy.TypeEnemy> EnemyAttached = new();
    ...
    void Start()
    {
        ...
        EnemyAttached.Add(Enemy.TypeEnemy.Default);
        EnemyAttached.Add(Enemy.TypeEnemy.Boss);
        _currentAttackCoolDown = attackCoolDown;
        attackCoolDownBar.SetMaxValue(attackCoolDown);
        attackDamage = Player.MainPlayer.Damage;
    }
    void Update()
    {
    ...
    private void Attack(Collider2D[] hitEnemies)
    {
        ...
        {
            Enemy enemy1 = enemy.GetComponentInParent<Enemy>();
            if (enemy1 && EnemyAttached.Contains(enemy1.EnemyType))
            {
