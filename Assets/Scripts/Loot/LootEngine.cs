...
public class LootEngine : MonoBehaviour 
{
    ...
    {
        if (_loot == null) { Debug.LogWarning("Object reference not set to an instance of an object"); return; }
        ILoot loot = (ILoot)_loot.Clone();
        GameObject go = Instantiate(Resources.Load<GameObject>($"Loot\\{loot.Name}"), vector, Quaternion.identity);
